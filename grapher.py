...
class Grapher:
    ...
    def plot_interactions(self, ax1, dates, prices, actions, rewards):
        ax1.plot(dates, prices, color='darkslategray', label='Prices')
        ax1.set_ylabel('Prices', color='darkslategray')
        if self.pred is not None:
            ax1.plot(dates, self.pred, color='darkseagreen', label='Pred')

        for i in range(np.max(actions)+1):
            label = self.action_labels[i] if i < len(self.action_labels) else i
            ax1.plot(dates[actions==i], prices[actions==i], marker='.', linestyle='None', label=label)
        ax1.legend()
    ...
    def show(self, action_labels=[], **kwargs):
